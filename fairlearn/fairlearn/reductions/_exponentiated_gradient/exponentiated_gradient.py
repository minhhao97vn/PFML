...
class ExponentiatedGradientResult:
    ...
    def __init__(self, best_classifier, best_gap, classifiers, weights, sample_weights, last_t, best_t,
                 n_oracle_calls):
...
class ExponentiatedGradient(Reduction):
    ...
    def fit(self, X, y, sensitive_features, sw=None, **kwargs):
        ...
        if sw is None:
            _sw = pd.Series(np.ones(len(y_train)))
        else:
            _sw = pd.Series(sw)
        n = X_train.shape[0]
        ...
        lagrangian = _Lagrangian(X_train, A, y_train, self._estimator, self._constraints,
                                 self._eps, B, sw=_sw, opt_lambda=True, **kwargs)
        ...
        for t in range(0, self._T):
            ...
            if t == 0:
                if self._nu is None:
                    _s = (pred_h - y_train).abs().multiply(_sw)
                    _m = _s.sum()/_sw.sum()
                    _ds = (_s-_m).pow(2).multiply(_sw)
                    _std = np.sqrt(_ds.sum()/_sw.sum())
                    self._nu = _ACCURACY_MUL * _std / np.sqrt(n)
            ...
            if t == 0 or not _RUN_LP_STEP:
                ...
                Q_LP, lambda_LP, result_LP = lagrangian.solve_linprog(self._nu)
            ...
            if gap_EG < gap_LP:
                Qs.append(Q_EG)
                gaps.append(gap_EG)
                save_lambda = lambda_EG
            else:
                Qs.append(Q_LP)
                gaps.append(gap_LP)
                save_lambda = lambda_LP
        ...
        self._weights = self._expgrad_result._weights
        self._sample_weights = self._expgrad_result._sample_weights
        self._lambda = save_lambda
